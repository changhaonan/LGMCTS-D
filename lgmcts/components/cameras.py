...
def get_agent_cam_config(image_size: tuple[int, int] = (128, 256)):
    ...
    if height == 128 and width == 256:
        ...
        return {
        "front": NearPerfectCamera256x256.CONFIG[0],
        "top": NearPerfectCamera256x256.CONFIG[1],
    }
    else:
        raise ValueError(f"Please add camera configs of image size (128,256) or (256, 256), because the camera are tuned for this size; "
...
class NearPerfectCamera256x256(object):
    """Top-down noiseless image used only by the oracle demonstrator."""
