"""
Spatial utils from NotoinGraph project
...
class Points9:
    ...
    # Interface methods
    @staticmethod
    def from_pcd(pcd: Union[o3d.geometry.PointCloud, np.ndarray]):
    ...
    @staticmethod
    def from_bbox(pos: np.ndarray, size: np.ndarray, center: Union[np.ndarray, None] = None):
    ...
    @staticmethod
    def from_aabb(aabb_min: np.ndarray, aabb_max: np.ndarray):
        """Parse the axis-aligned bounding box into 9 points."""
        xc, yc, zc = (aabb_min + aabb_max) / 2
        xl, yf, zl = aabb_min
        xr, yb, zu = aabb_max
        return np.array([xc, yc, zc, xl, yf, zl, xr, yb, zu])
